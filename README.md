...
Google Forms 스타일 UI · 자동채점 · 자가채점 · 챕터별 결과 분석 · 시험 기록 관리 · 경과 시간 측정 · 즐겨찾기
...
| **경과 시간 측정** | 시험 시작과 동시에 헤더에 경과 시간 표시 (MM:SS / HH:MM:SS) |
| **즐겨찾기** | 시험 중 문제 카드 우상단 ★ 버튼으로 저장, `favorites.html`에서 모아보기 및 개별 해제 |
...
```
index.html      시작 화면 (결과 폴더 설정, 시험 시작, 즐겨찾기, 문제 관리)
exam.html       시험 진행 화면 (진행바, 경과 시간, 이전/다음, 정답 토글, 메모, 패스, 즐겨찾기)
result.html     채점 결과 + 시험 기록
manage.html     문제 관리 (조회·추가·수정·삭제, 15개씩 페이지네이션)
favorites.html  즐겨찾기 문제 모아보기
```
...
veda-test-c/
├── index.html             시작 화면
├── exam.html              시험 진행 화면 (타이머, 즐겨찾기 버튼 포함)
├── result.html            결과 및 기록 화면
├── manage.html            문제 관리 화면
├── favorites.html         즐겨찾기 문제 모아보기
├── app.js                 전체 로직 (문제 로딩·채점·저장·타이머·즐겨찾기)
...
#### 경과 시간 측정
- 시험 시작 버튼을 누르면 시험 헤더에 경과 시간 표시 (MM:SS, 1시간 이상 시 HH:MM:SS)
- 제출 / 그만보기 시 타이머 자동 정지

#### 즐겨찾기
- 시험 중 문제 카드 우상단 ★ 버튼으로 즐겨찾기 추가·해제
- 즐겨찾기한 문제는 `favorites.html`에서 모아보기 가능
- 홈 화면에 `★ 즐겨찾기` 버튼 추가
- 즐겨찾기 데이터는 localStorage에 저장 (JSON 파일 구조에 영향 없음)
- `favorites.html`에서 각 문제별로 개별 즐겨찾기 해제 가능
